...
print("🔧 Using existing packages - no version conflicts!")
...
try:
    ...
    try:
        ...
        print("✅ EarlyStoppingCallback available")
    except:
        EARLY_STOPPING_AVAILABLE = False
        print("⚠️ EarlyStoppingCallback not available")
...
if not TRANSFORMERS_OK:
    raise RuntimeError("❌ Cannot proceed without transformers library")
...
print("🚀 BULLETPROOF MEMOTION 3.0 - ENHANCED PERFORMANCE VERSION")
print("=" * 60)
...
def extract_clip_features_safely(df, image_folder, dataset_name):
    ...
    print(f"🖼️ Computing bulletproof {dataset_name} CLIP features...")
...
class BulletproofVisualBERT(nn.Module):
    def __init__(self, num_classes=2, class_weights=None):
        ...
        print("🧠 Initializing Bulletproof VisualBERT...")
        ...
        if config.USE_FOCAL_LOSS and class_weights is not None:
            ...
            print("✅ Using Bulletproof Focal Loss")
        else:
        ...
        print(f"✅ Bulletproof model initialized with {sum(p.numel() for p in self.parameters() if p.requires_grad):,} parameters")
...
class BulletproofMemotionDataset(Dataset):
    def __init__(self, df, tokenizer, features_dict, max_length=256, is_test=False):
        ...
        print(f"✅ Bulletproof dataset initialized with {len(self.samples)} samples")
...
def run_bulletproof_pipeline():
    ...
    print("🚀 STARTING BULLETPROOF MEMOTION PIPELINE")
    print("=" * 60)
    ...
    try:
        ...
        print("🖼️ Extracting bulletproof visual features...")
        ...
        print("🔤 Initializing bulletproof tokenizer...")
        tokenizer = XLMRobertaTokenizer.from_pretrained(config.MULTILINGUAL_TOKENIZER)
        
        print("📚 Creating bulletproof datasets...")
        ...
        if len(unique_labels) > 1:
            ...
            print(f"⚖️ Bulletproof class weights: {dict(zip(unique_labels, class_weights))}")
        else:
            class_weights = None
            print("⚠️ Only one class found, using uniform weights")
        
        print("🧠 Initializing bulletproof model...")
        ...
        print("🧪 Testing bulletproof forward pass...")
        try:
            ...
            print(f"✅ Bulletproof forward pass successful: {output['logits'].shape}")
        except Exception as e:
        ...
        print(f"⚙️ Bulletproof hyperparameters:")
        print(f"   📚 Max length: {config.MAX_TEXT_LENGTH}")
        print(f"   🎛️ Learning rate: {config.LEARNING_RATE}")
        print(f"   🛡️ Dropout: {config.DROPOUT_RATE}")
        print(f"   ⏰ Epochs: {config.NUM_EPOCHS}")
        ...
        print("✅ Bulletproof training completed!")
        
        eval_results = {}
        if val_dataset:
            print("📊 Bulletproof evaluation...")
            eval_results = trainer.evaluate()
            
            print("📈 BULLETPROOF EVALUATION RESULTS:")
        ...
        print(f"💾 Bulletproof model saved to: {best_model_path}")
        
        if test_dataset:
            print("🎯 Generating bulletproof test predictions...")
            ...
            print("🎯 BULLETPROOF TEST PREDICTION SUMMARY:")
            ...
            print(f"   💾 Bulletproof predictions saved to: {predictions_file}")
        ...
        print(f"❌ Bulletproof pipeline error: {e}")
        traceback.print_exc()
        return None, None, None, None, None

# Execute Bulletproof Pipeline
if __name__ == "__main__":
    print("🌟 INITIALIZING BULLETPROOF MEMOTION 3.0 PIPELINE")
    print("✅ Works with ANY transformers version - no dependency conflicts")
    ...
    if trainer is not None:
        print("\n🎉 BULLETPROOF PIPELINE COMPLETED SUCCESSFULLY!")
        ...
        print("✅ 100% BULLETPROOF - WORKS WITH ANY SETUP!")
    else:
        print("\n❌ Bulletproof pipeline failed. Check the error messages above.")